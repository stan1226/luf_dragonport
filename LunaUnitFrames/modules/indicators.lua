...
local Indicators = {
	list = {
		...
		["dragonport"] = {"Interface\\AddOns\\LunaUnitFrames\\media\\textures\\UI-DialogBox-Gold-Dragon"},
		["dragoneliteport"] = {"Interface\\AddOns\\LunaUnitFrames\\media\\textures\\UI-DialogBox-Gold-Dragon"},
	},
}
...
local function UpdateDragonport(enabled, indicator, unit, unitGroup)
	if not enabled then
		indicator:Hide()
	else
		local texture
...
local function UpdateDragoneliteport(enabled, indicator, unit, unitGroup)
	if not enabled then
		indicator:Hide()
	else
		local texture
...
function Indicators:OnEnable(frame)
	if not frame.indicators then
		...
		for name in pairs(LunaUF.db.profile.units[frame.unitGroup].indicators.icons) do
			if name == "elite"  or name == "dragonport" or name == "dragoneliteport" then
...
function Indicators:FullUpdate(frame)
	...
	if frame.indicators.dragonport then UpdateDragonport(config.dragonport.enabled, frame.indicators.dragonport, frame.unit, frame.unitGroup) end
	if frame.indicators.dragoneliteport then UpdateDragoneliteport(config.dragoneliteport.enabled, frame.indicators.dragoneliteport, frame.unit, frame.unitGroup) end
